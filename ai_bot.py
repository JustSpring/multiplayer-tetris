...
class Game:
    ...
    def print_start(self):
        ...
        while not self.start:
            ...
            self._draw_start_screen(
                ip_rect, name_rect, button, start_rect,
                color_active, color_passive, active_ip, active_name, active_connect, active_start, font
            )
            if active_connect:
    ...
    def _draw_start_screen(
        self, ip_rect, name_rect, button, start_rect,
        color_active, color_passive, active_ip, active_name, active_connect, active_start, font
    ):
        screen.fill((180, 180, 180))
...
class TetrisAI:
    def __init__(self):
        self.weights = {"aggregate_height": -0.510066,
                        "complete_lines": 0.760666,
                        "holes": -0.35663,
                        "bumpiness": -0.184483}

    def get_combine_cells(self, grid, piece):
        final_cells = copy.deepcopy(grid)
        y = piece.position[1]
        for posY in range(3, -1, -1):
            x = piece.position[0]
            for posX in range(4):
                if piece.getPiece()[posY][posX] == 1:
                    final_cells[y][x] = piece.shape.color
                x += 1
            y += 1
        return final_cells
    ...
    def calculate_heuristic(self, grid):
        if not grid:
            print("error")
        aggregate_height = sum(self.get_column_height(grid, column) for column in range(WIDTH_G))
        complete_lines = sum(1 for row in grid if all(column != 0 for column in row))
        holes = aggregate_height - sum(1 for column in range(WIDTH_G) for row in range(HEIGHT_G) if grid[row][column] != 0)
        bumpiness = sum(abs(self.get_column_height(grid, column) - self.get_column_height(grid, column + 1)) for column in range(WIDTH_G - 1))
        return {"aggregate_height": aggregate_height, "complete_lines": complete_lines, "holes": holes, "bumpiness": bumpiness}

    def calculate_score(self, new_grid):
        score = 0
        heuristics = self.calculate_heuristic(new_grid)
        for key in heuristics:
            score += heuristics[key] * self.weights[key]
        return score

    def check_collision(self, grid, piece):
        posY = 0
        for y in range(3, -1, -1):
            posX = 0
            for x in range(4):
                if piece.getPiece()[y][x] != 0 and (
                        piece.position[0] + posX < 0 or piece.position[0] + posX >= WIDTH_G or
                        piece.position[1] + posY < 0 or piece.position[1] + posY >= HEIGHT_G or
                        grid[piece.position[1] + posY][piece.position[0] + posX] != 0):
                    return False
                posX += 1
            posY += 1
        return True

    def simulate_move(self, grid, piece):
        if not self.check_collision(grid, piece):
            return None
        while self.check_collision(grid, piece):
            piece.position = (piece.position[0], piece.position[1] - 1)
        piece.position = (piece.position[0], piece.position[1] + 1)
        return self.get_combine_cells(grid, piece)

    def choose_best_move(self, grid, current_piece):
        piece = Piece(current_piece.shape)
        best_score = -float('inf')
        best_move = None
        best_grid = None
        for rotation in range(4):
            for x in range(-2, WIDTH_G + 2):
                piece.position = (x, piece.getStartPosition()[1])
                piece.rotation = rotation
                new_grid = self.simulate_move(grid, piece)
                if new_grid:
                    score = self.calculate_score(new_grid)
                    if score > best_score:
                        best_grid = copy.deepcopy(new_grid)
                        best_score = score
                        best_move = (rotation, x)
        self.calculate_score(best_grid)
        return best_move
...
while True:
    ...
    if game.alive:
        ...
        if time.time() - start_timeD > 0.2:
            r, x = ai.choose_best_move(copy.deepcopy(game.cells), copy.deepcopy(game.piece))
            start_timeD = time.time()
            game.piece.position = (x, game.piece.position[1])
            game.piece.rotation = r
            go_down = True

        if go_down and time.time() - start_timeD > 0.1:
            go_down = False
            game.allDown()
            game.checkAndRemoveRow()
